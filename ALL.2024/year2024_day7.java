	...
	public static void main(String [] args) {
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			for (int kk = 0; kk < end; kk++) { // 15 in decimal is 1111 in binary
				...
				//out.print("Integer.toString(2)): "); out.println(Integer.toString(kk, 2));
				String binary = padLeftZeros(Integer.toString(kk, 2), 20);
				//String binary = String.format("%020d", Integer.toString(kk, 2));
				//out.println(binary);
				String rev = new StringBuilder(binary).reverse().toString();
				BigInteger val = var_ints.get(0);
				for (int jj = 1; jj < var_ints.size(); jj++) {
					if (rev.charAt(jj-1) == '0') {
