...
class year2024_day2_2 {
	public static void main(String [] args) {
		out.println("		2024 Day2.2");
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			Vector <Integer> var_intsOrig = new Vector<>(var_ints);

			int bad = 0;
			bad = checkVec(var_ints);
			if (bad == 0) {
				count++;
				//for (Integer num: var_ints) { out.print(num); } out.println();
			} else {
				for (int iii = 0; iii < var_intsOrig.size(); iii++) {
					var_ints = new Vector<>(var_intsOrig);
					var_ints.remove(iii);
					bad = checkVec(var_ints);
					if (bad == 0) {
						//for (Integer num: var_ints) { out.print(num); } out.println();
	...
	public static int checkVec(Vector <Integer> var_ints) {
		int bad = 0;
		int none = 0;
		int dec = 0;
		int inc = 0;
