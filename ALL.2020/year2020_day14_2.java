		...
		*/
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			if (m.find()) {
				...
				int val = Integer.valueOf(m2.group(1));
				out.println(val);
				...
				StringBuilder bin1Orig = new StringBuilder(bin1);
				...
				out.println(countX);
				int top = (int)Math.pow(2, countX);
				for (int ii = 0; ii < top; ii++) {
					bin1 = new StringBuilder(bin1Orig);
					String xx2 = Integer.toString(ii, 2);
					StringBuilder bin3 = new StringBuilder();
					while (bin3.length() + xx2.length() < countX) {
						bin3.append('0');
					}
					bin3.append(xx2);
					for (int kk = 0, ll = 0; kk < key.length(); kk++) {
						if (key.charAt(kk) == 'X') {
							bin1.setCharAt(kk, bin3.charAt(ll));
							ll++;
						}
					}
					out.println(bin1);
					String yy = bin1.toString();
					long yyaddr = Long.parseLong(yy, 2);
					mp.put(yyaddr, Long.parseLong(m2.group(2)));
				}
				out.println("-----------");
