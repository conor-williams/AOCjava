...
class year2020_day9_2 {
	public static int [][] keypad = {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}};
	public static void main(String [] args) {
		out.println("		2020 Day9.2");
		...
		//out.print("ans: "); out.println(ans);
		long ansans = 0;
after:
		for (int i = 0; i < blah.size(); i++) {
			long startNum = Long.parseLong(blah.get(i));
			long sum = startNum;
			int coStart = i;
			int co = i+1;
			Vector <Long> nu = new Vector<>();
			nu.add(startNum);
			while (1 == 1) {
				long ne = Long.parseLong(blah.get(co));
				nu.add(ne);
				sum += ne;
				//out.println(sum);
				if (sum == ans) {
					ansans = Collections.min(nu) + Collections.max(nu);
					break after;
				} else if (sum < ans) {
					co++;
					continue;
