...
//		System.setOut(new PrintStream(new java.io.OutputStream() { public void write(int b) { } }));
		...
		*/
		...
		int secS = 2503;
		int score[] = new int[mp.size()];
		for (int sec = 1; sec <= secS; sec++) {
			int pos = 0;
			int roundWinner = 0;
			int max = 0;
			int dists[] = new int[mp.size()];
			for (var entry : mp.entrySet()) {
				...
				//out.print("speed: "); out.println(speed);
				//out.print("flytime: "); out.println(flytime);
				//out.print("rest: "); out.println(rest);
				int dist1 = speed * flytime * (sec / (flytime+rest));
				int left = sec % (flytime+rest);
				//out.println("dist1: "); out.println(dist1);
				//out.println("left: "); out.println(left);
				...
				//out.println(dist1);
				//if (dist1 > max) {max = dist1; roundWinner = pos; }
				dists[pos] = dist1;
				pos++;
			}
			int ma = Arrays.stream(dists).max().orElseThrow();
		...
		out.println("score: ");
		for (int ii = 0; ii < mp.size(); ii++) {
			out.println(score[ii]);
		}
		*/
//		System.setOut(originalOut);
		out.print("**j_ans: ");
		out.print(Arrays.stream(score).max().orElseThrow());
		out.println("");
	}
}
