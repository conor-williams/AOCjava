...
class year2015_day12_2 {
	...
	public static void main(String [] args) {
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			//ne = ne.replaceAll("\\{[^\\{\\[]*red[^\\}\\]]*\\}", "");
			//ne = ne.replaceAll("\\{[^\\{]*red[^\\}]*\\}", "");

			out.println(ne);
			int pos = 0;
			while ((pos = ne.indexOf("red", pos+1)) != -1) {
				out.println(pos);
				int level = 0;
				//look left
				int ba1 = 0;
				int startPos = 0;
after:
				for (int yy = pos-1; yy >= 0; yy--) {
					char ch = ne.charAt(yy);
					if (ch == '[') {
						level++;
						if (level == 1) {ba1 = 1; break after;}
					} else if (ch == ']') {
						level--;
					} else if (ch == '{') {
						level++;
						if (level == 1) {startPos = yy; break after;}
					} else if (ch == '}') {
						level--;
					}
				}
				...
				int endPos = 0;
after2:
				for (int yy = pos+3; yy < ne.length(); yy++) {
					char ch = ne.charAt(yy);
					if (ch == '[') {
						level++;
					} else if (ch == ']') {
						level--;
						if (level == -1) {ba2 = 1; break after2;}
					} else if (ch == '{') {
						level++;
					} else if (ch == '}') {
						level--;
						if (level == -1) {endPos = yy; break after2;}
					}

				}
				if (ba2 == 1) {continue;}
				String commas = ",".repeat(endPos-startPos+1);
				String ne10 = ne.substring(0, startPos) + commas + ne.substring(endPos+1);
				assert ne10.length() == ne.length();
				ne = new String(ne10);
			}
