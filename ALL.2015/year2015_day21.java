...
class year2015_day21 {
	...
	public static void main(String [] args) {
		...
		int minPlayerCost = Integer.MAX_VALUE;
		...
		for (int we = 0 ; we < weaponsTot ; we++) {
			...
			for (int ar = -1; ar < armorTot ; ar++) {
				...
				for (int end = 0; end <= allowedUpTo; end++) {
					...
					int arrOrig[] = Arrays.copyOf(arr, arr.length);
					...
					do {
						...
						nextPermutation(arr);
					} while (!Arrays.equals(arr, arrOrig));
