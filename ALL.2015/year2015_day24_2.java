			...
			do {
				...
				if (mysum == weTot && (mymin == Long.MAX_VALUE || num1 <= mymin)) {
					...
					for (int end1 = 0; end1 < pos; end1++) {
						...
						int arr11[] = new int[pos];
						for (int ii = 0; ii < pos; ii++) {
							arr11[ii] = arr1[ii];
						}
						Arrays.sort(arr11);
						int arr1Orig[] = new int[pos];
						arr1Orig = Arrays.copyOf(arr11, pos);
						...
						do {
							...
							if (mysum1 == weTot) {
								int pos2 = 0; int tmpW2[] = new int[leny];
								for (int k = 0; k < pos; k++) {
									if (arr11[k] == 0) {
								...
								for (int end2 = 0; end2 < pos2; end2++) {
									...
									int arr22[] = new int[pos2];
									for (int ii = 0; ii < pos2; ii++) {
										arr22[ii] = arr2[ii];
									}
									Arrays.sort(arr22);
									int arr2Orig[] = new int[pos2];
									arr2Orig = Arrays.copyOf(arr22, pos2);
									do {
										int mysum2 = 0;
									...
									} while (nextPermutation(arr22) && !Arrays.equals(arr22, arr2Orig));
								}

							}
						} while (nextPermutation(arr11) && !Arrays.equals(arr11, arr1Orig));
					}
				}
