...
class year2021_day16_2 {
	...
	public static Vector<Long> vec[] = new Vector[2000];
	public static void main(String [] args) {
		out.println("		2021 Day16.2");
		out.flush();
	...
	public static int getitit(String bitLineS, int subCounter) {
		...
		int found = 0;
		for (int zz = 0; zz < bitLineS.length(); zz++) {
			if (bitLineS.charAt(zz) == '1') {
				found = 1; break;
			}
		}
		if (found == 0) {ind--; return subCounter;}
		...
		if (mp2.get(type) == 4) { //literal packet
			...
			if (literal.length() > 0) {
				...
				vec[ind].add(Long.parseLong(lit, 2));
			...
			if (I == 0) {
				...
				long N = Long.parseLong(bitLineS.substring(7, 7+11), 2);
				int st = 18; 
				bitLineOrigPos += 18;
				int subC = 18;
			...
			out.print("apply maths: ");
			for (var var_int: vec[ind+1]) {
				out.print(var_int); out.print(" ");
			}
			out.println();
			//Scanner scanner = new Scanner(System.in); scanner.nextLine();
			long ans;
			switch(mp2.get(type)) {
				...
				case 5: {
						ans = (vec[ind+1].get(0).compareTo(vec[ind+1].get(1)) > 0 ? 1  : 0);
						vec[ind].add(ans);
						break;
				}
				case 6: {
