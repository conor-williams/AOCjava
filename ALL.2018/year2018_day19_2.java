		...
		if (false) {
			...
			int regb[] = new int[10];
			{
				...
				for (int k = 0; k < instNum; k++) {	
					...
					Matcher m = p.matcher(instr[k]);
					int ret = 0;
					if (m.find()) {
						ret = 4;
						four = m.group(1);
						regA = Integer.valueOf(m.group(2));
						regB = Integer.valueOf(m.group(3));
						regC = Integer.valueOf(m.group(4));
					}
