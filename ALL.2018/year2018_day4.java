...
class year2018_day4 {
	...
	public static void main(String [] args) {
		...
		int themaxmin = 0; 
		{
			int [] xx = mpMins.get(guardG);

			for (int ii = 0; ii < 60; ii++) {
				if (xx[ii] > max) {max = xx[ii]; themaxmin = ii;}
			}

				/*
			out.print("xx"); out.println(xx[0]);
			Integer[] yy = Arrays.stream(xx).boxed().toArray(Integer[]::new);
			Arrays.sort(yy, Collections.reverseOrder());
			out.println(Arrays.toString(yy));
			maxmins = yy[0];
			*/
		}
