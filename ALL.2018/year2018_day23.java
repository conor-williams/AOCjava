...
//		System.setOut(new PrintStream(new java.io.OutputStream() { public void write(int b) { } }));
		...
		*/
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			m.find();
			//out.println(p); out.println(blah.get(i));
		...
		for (int i = 0; i < leny; i++) {
			if (Math.abs(xyzr[i].r1) > max) {max = Math.abs(xyzr[i].r1); maxi = i;}
		}
		for (int i = 0; i < leny; i++) {
			if (maxi == i) {tot++; continue;}
			if ((Math.abs(xyzr[i].x1 -xyzr[maxi].x1) + Math.abs(xyzr[i].y1 - xyzr[maxi].y1) + Math.abs(xyzr[i].z1 - xyzr[maxi].z1)) <= max) {
                        tot++;
			}
		}
