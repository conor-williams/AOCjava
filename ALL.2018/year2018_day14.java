...
//		System.setOut(new PrintStream(new java.io.OutputStream() { public void write(int b) { } }));
		...
		while (var_ints.size() <= after+10) {
			int val = var_ints.get(pos1) + var_ints.get(pos2);
			//out.println(val);
			if (val < 10) {
				var_ints.add(val);
			} else {
				...
				var_ints.add(tens);
				var_ints.add(singles);
			}
			int timesPos1 = var_ints.get(pos1) + 1 % var_ints.size();
			int timesPos2 = var_ints.get(pos2) + 1 % var_ints.size(); 
		...
		Vector <Integer> var_ans = new Vector<>();
		//String var_ans = new String("");
		for (int ii = 0; ii < 10; ii++) {
			var_ans.add(var_ints.get(after+ii));
		}
