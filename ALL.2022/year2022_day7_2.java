...
class year2022_day7_2 {
	...
	public static void main(String [] args) {
		...
		/*
		BigInteger totSlash = zero;
		var ve = mpAft.get("/");
		for (int ii = 0; ii < ve.size(); ii++) {
			totSlash = totSlash.add(calcUnder(ve.get(ii)));
		}
		BigInteger left = diskSize.subtract(totSlash);
		*/

		out.println("filesystem:");
		BigInteger totSlash = zero;
		for (Map.Entry<String, fileordir> entry : mp.entrySet()) {
			var ee = entry.getValue();
			if (ee.type == 'F') {
				totSlash = totSlash.add(ee.size);
			}
			//System.out.println("Key: " + entry.getKey() + ", Value: " + entry.getValue());
		}
		out.println(totSlash);
		BigInteger diskSize = new BigInteger("70000000");
		BigInteger unused = diskSize.subtract(totSlash);
		out.print("unused: "); out.println(unused);
		BigInteger want = new BigInteger("30000000").subtract(unused);
		out.print("want: "); out.println(want);

		out.println("----------");
		BigInteger tottot = zero;
		BigInteger min = new BigInteger("99999999999999");
		for (Map.Entry<String, Vector<String>> entry: mpAft.entrySet()) {
			var ke = entry.getKey();
			//out.println(ke);
			//Scanner scanner = new Scanner(System.in); scanner.nextLine();
			...
			out.println(back);
			if (back.compareTo(want) >= 0) {
				if (back.compareTo(min) < 0) {min = back;}
			}
		}
	...
	public static BigInteger calcUnder(String df) {
		...
		//for (Map.Entry<String, Vector<String>> entry: mpAft.entrySet()) 
		{
			...
			var ve = mpAft.get(df);
			for (int ii = 0; ii < ve.size(); ii++) {
				//out.println(df); out.println(ve.get(ii));
