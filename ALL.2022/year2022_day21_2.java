...
class year2022_day21_2 {
	...
	static String restRootOrig1 = new String();
	static String restRootOrig2 = new String();
	static Pattern p2 = Pattern.compile("(\\d+)\\s+(\\S)\\s+(\\d+)");
	static Map <String, Tuple<String, Integer>> mp = new HashMap<>();
	static Map <String, Tuple<String, Integer>> mpOrig = new HashMap<>();
	static class inst_s {
		String s1;
		String s2;
		String s3;
	...
	public static void main(String [] args) {
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			if (inst[i].s1.equals("root")) {
				Matcher m3 = p3.matcher(rest);
				m3.find();
				restRootOrig1 = m3.group(1);
				restRootOrig2 = m3.group(2);
			}

		}
		mpOrig = new HashMap(mp);
		String newString = new String();
		long upper = (long)Math.pow(10, 13);
		long lower = 0L;
		//int five = 0;
		long ans = 0;
		Map <Long, Boolean> mpTried = new HashMap<>();
...
aft3:		
		while (true) {
			...
			/*
			if (upper-lower <= 20) {
				//out.println(upper); out.println(lower); out.println(five);
				//iii = lower+five;
				iii = lower;
				while (true) {
					if (mpTried.get(iii) == null) {
						mpTried.put(iii, true);
						break;
					} else {
						iii++;
					}
				}
				//out.print("trying: "); out.println(iii);
				//five = 0;
			*/
			if (checking) {
				iii--;
			...
			String tmpS = "" + iii;
			mp.put("humn", new Tuple(tmpS, mp.get("humn").second));
...
after:
				for (var en1: mp.entrySet()) {
					if (en1.getValue().second == 0 && alldigitsAndOp(en1.getValue().first) == 1 && hasOp(en1.getValue().first) == 1) {
						...
						if (xx1.equals("-99")) {lower+=100; go = 1; break nextiii;}
					} else if (en1.getValue().second == 0 && alldigits(en1.getValue().first) == 1) {
						for (var en2: mp.entrySet()) {
							if ((en2.getValue().first.indexOf(en1.getKey())) != -1) {
				...
				if (alldigits(mp.get("root").first) == 1) {
					long one = Long.parseLong(mp.get(restRootOrig1).first);
					long two = Long.parseLong(mp.get(restRootOrig2).first);

					if (mp.get(restRootOrig1).equals(mp.get(restRootOrig2))) {
						if (!checking) {checking = true;}
						mpTried.put(iii, true);
					       	//out.println("found end..."); out.println(iii);
						if (mpTried.get(iii-1) != null && mpTried.get(iii-1) == false) {
							ans = iii;
							break aft3;
						}
					} else if (one < two) {
						mpTried.put(iii, false);
						if (checking) {
							if (mpTried.get(iii+1) == true) {
								ans = iii+1;
								break aft3;
							}
						} else {
	...
	static String doCalc(String ch) {
		...
		if (!m2.find()) {out.println(p2); out.println(ch); out.println("ERR"); return "-99";}
