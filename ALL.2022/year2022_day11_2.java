...
class year2022_day11_2 {
	...
	public static void main(String [] args) {
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			if (m.find()) {
				...
				while (scanner.hasNext()) {
					...
					var_ints.add(new BigInteger(ne2));
				}
				...
				monkeys[monkeyNum].divby = new BigInteger(m4.group(1));
			} else if (m5.find()) {
		...
		*/
		...
		BigInteger supermodulo = one;
	        for (int i = 0; i < monkeyNum; i++) {
       		         supermodulo = supermodulo.multiply(monkeys[i].divby);
        	}

		out.print("Super: ");
		out.println(supermodulo);
		for (int ro = 0; ro < 10000; ro++) {
			for (int ii = 0; ii < monkeyNum; ii++) {
				for (int zz = 0; zz < monkeys[ii].starts.size(); zz++) {
					monkeys[ii].times++;
					BigInteger val = monkeys[ii].starts.get(zz);
					...
					if (monkeys[ii].opVal.equals("old")) {
						...
						vv = new BigInteger(monkeys[ii].opVal);
					}

					if (monkeys[ii].plusmul.equals("+")) {
						///out.print(val); out.print(" + "); out.println(vv);
						val  = val.add(vv);
					...
					BigInteger valval = val.mod(supermodulo);

					//out.println(val); out.println(valval);
					if (val.mod(monkeys[ii].divby).equals(zero)) {
						int to = monkeys[ii].monkeytrue;
						monkeys[to].starts.add(valval);
					} else {
			...
			/*
			if (ro == 0 || ro == 19 || ro == 999) {
			out.print("round"); out.println(ro);
			for (int ii = 0; ii < monkeyNum; ii++) {
				out.print("monkey"); out.print(ii); out.print(" "); out.println(monkeys[ii].times);
			}
			}
			*/
		}
		Vector <Long> ti = new Vector<>();
		for (int ii = 0; ii < monkeyNum; ii++) {
