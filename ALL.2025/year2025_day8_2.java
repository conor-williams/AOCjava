...
after22:
		for (int ii = 0, n = jboxdists.size(); ii < n; ii++) {
			...
			for (int zz = 0; zz < groups.size(); zz++) {
				...
				if (firInGroup && secInGroup) {
					...
					if (xx.size() == size) {
						out.println(fir);
						out.println(sec);
						ans = BigInteger.valueOf(fir.first).multiply(BigInteger.valueOf(sec.first));
						break after22;
					}
					isFound = true;
					break;
