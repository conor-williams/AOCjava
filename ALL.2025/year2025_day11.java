...
//	        public static int maxPath = 0;
	...
	public static Map <String, Vector <String>> mp;
	public static void main(String [] args) {
		out.println("		2025 Day11.1");
		out.flush();
		...
		*/
		...
		for (int i = 0, nn = blah.size(); i < nn; i++) {
			...
			Vector <String> var_str = new Vector<>();
			while (scanner.hasNext()) {
				String ne = scanner.next();
			   	var_str.add(ne);
			}
			mp.put(m.group(1), var_str);
		}
		out.println("---------");
		String st = "you";
	...
	public static void loop(String it) {
		var ve = mp.get(it);
		for (var item: ve) {
			//out.println("item: " + item);
			if (item.equals("out")) {
				out.println("path ended...");
				numPaths++;
				return;
			} 
			loop(item);
