...
//		System.setOut(new PrintStream(new java.io.OutputStream() { public void write(int b) { } }));
		...
		*/
		...
		for (int i = 0, nn = blah.size(); i < nn; i++) {
			...
			m.find();
			Tuple<Integer, Integer> po = new Tuple(Integer.valueOf(m.group(1)), Integer.valueOf(m.group(2)));
			points.add(po);
		}
		int en = points.size();
		long max = 0;
		for (int ii = 0; ii < en-1; ii++) {
			var xx1 = points.get(ii).first;
			var yy1 = points.get(ii).second;
			for (int jj = ii+1; jj < en; jj++) {
				var xx2 = points.get(jj).first;
				var yy2 = points.get(jj).second;
