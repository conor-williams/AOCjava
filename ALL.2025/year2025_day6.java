...
class year2025_day6 {
	...
	public static void main(String [] args) {
		...
		//out.println();
		//out.println("row size: " + sums.size());
		for (int col = 0; col < sums.size(); col++) {
			Vector <Integer> toSum = new Vector<>();
			for (int ii = 0; ii < ve.size()-1; ii++) {
				var vi = ve.get(ii);
				//sum = Integer.valueOf(vi.get(col));
				//out.print(vi.get(col));
				toSum.add(Integer.valueOf(vi.get(col)));
				//out.print(" ");
				//out.print(sums.get(col));
			}
			long sum = 0;
			if (sums.get(col).equals("+")) {
				sum = 0;
				for (int ii = 0; ii < toSum.size(); ii++) {
					sum += Integer.valueOf(toSum.get(ii));
				}
