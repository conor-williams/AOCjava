	...
	public static void main(String [] args) {
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			while (scanner.hasNext()) {
				...
				if (from.length() % 2 == 1 && to.length() == from.length()) {
				...
				if (from.length() % 2 == 1) {
					...
					sb.append('0');
					from = sb.toString();
					System.out.println("modified: " + from + " -> " + to);
				}

				String subFrom = from.substring(0, from.length()/2);
				String subTo = new String();
				if (to.length() % 2 == 1) {
					subTo = to.substring(0, (to.length()/2) + 1);
				} else {
					subTo = to.substring(0, to.length()/2);
				}
				...
				for (long ii = from1; ii <= to1; ii++) {
					//out.println((ii * Math.pow(10, mul)) + ii);
					int mul = Long.toString(ii).length();
					long val = (ii * (long)Math.pow(10, mul)) + ii;
					out.println("val is: " + val);
					if (val >= fromLong && val <= toLong) {
						out.println("inv: " + val);
						numInv++;
						sum += val;
					}
