...
class year2016_day4_2 {
	...
	public static void main(String [] args) {
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			if (found == 0) {
				int vv = Integer.valueOf(partid) % 26;
				StringBuilder sb = new StringBuilder(part1);
				for (int zz = 0; zz < part1.length(); zz++) {
					sb.setCharAt(zz, (char)(((part1.charAt(zz) - 97 + vv) % 26) + 97));
				}
				///out.println(sb);
				String transstring = sb.toString();
				if (transstring.contains("north")) {
					tot+=Integer.valueOf(partid);
					break;
				}
			}
		}
