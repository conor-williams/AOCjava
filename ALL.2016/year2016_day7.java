	...
	public static void main(String [] args) {
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			for (int ii = 0; ii < partsGood.size(); ii++) {
				...
				for (int kk = 0; kk < pbG.length()-3; kk++) {
					if (pbG.charAt(kk) == pbG.charAt(kk+3) && pbG.charAt(kk+1) == pbG.charAt(kk+2) && pbG.charAt(kk) != pbG.charAt(kk+1) ){
						abbaGood.add(pbG.substring(kk, kk+4));
					}
				}
			}
			...
			for (int ii = 0; ii < partsBad.size(); ii++) {
				...
				for (int kk = 0; kk < pbB.length()-3; kk++) {
					if (pbB.charAt(kk) == pbB.charAt(kk+3) && pbB.charAt(kk+1) == pbB.charAt(kk+2) && pbB.charAt(kk) != pbB.charAt(kk+1)){
						abbaBad.add(pbB.substring(kk, kk+4));
					}
				}
			}
			if (abbaBad.size() > 0) {
			} else if (abbaGood.size() > 0) {
				tot++;
			} 
			/*
