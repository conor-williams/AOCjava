...
class year2016_day10 {
	public static int [][] keypad = {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}};
	public static void main(String [] args) {
		out.println("		2016 Day10.1");
...
after:
		while (1 == 1) {
			...
			for (int i = 0; i < blah.size(); i++) {
				String ne = blah.get(i);
				for (int kk = 0; kk <= maxBot; kk++) {
					Collections.sort(bots[kk]);
					if (bots[kk].size() < 2) {continue;}
					//if (bots[kk].get(0) == 2 && bots[kk].get(bots[kk].size()-1) == 5) 
					if (bots[kk].get(0) == 17 && bots[kk].get(bots[kk].size()-1) == 61) {
						ans = kk;
						break after;
					}
				}
