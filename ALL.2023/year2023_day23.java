...
class year2023_day23 {
	...
	public static void main(String [] args) {
		...
		//Vector <Tuple <Integer, Integer> allNodes = new Vector<>();
		allNodes = new Vector<>();
	...
	public static void find2(int[] start, int[] end) {
		...
		/*
		var xx = mp.get(new Tuple(start[0], start[1]));
		for (int ii = 0; ii < xx.size(); ii++) {
			openSet.add(new Node2(xx.get(ii).first, xx.get(ii).second, 0));
		}
		*/
	...
	public static int findAllPathsAStarred(int[] start, int[] end) {
		...
		while (!openSet.isEmpty()) {
			...
			if (current.x == end[0] && current.y == end[1]) {
				...
				//if (current.path > maxPath) {maxPath = current.path; out.println(current.path);}
			...
			for (int[] dir : new int[][]{{0, -1}, {1, 0}, {0, 1}, {-1, 0}}) {
				...
				if (newX >= 0 && newX < lenx && newY >= 0 && newY < leny && !closedSet[newY][newX]) {
					...
					//out.print(newDir); out.print(" "); out.print(grid[newY][newX]); out.print(" "); out.print(newX); out.print(" "); out.println(newY);
