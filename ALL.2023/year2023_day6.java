...
class year2023_day6 {
	...
	public static void main(String [] args) {
		...
		Vector <Integer> lasts = new Vector<>();
		...
		for (int i = 0; i < blah.size(); i++) {
			...
			while (scanner.hasNext()) {
				...
				if (timedist.charAt(0) == 'T') {
					lasts.add(Integer.valueOf(ne));
				} else {
					records.add(Integer.valueOf(ne));
				}
			}
		}
		///out.println(lasts.size());
		int tot = 1;
		for (int ii = 0; ii < lasts.size(); ii++) {
			int tmptot = 0;
			for (int hold = 1; hold < lasts.get(ii)-1; hold++) {
				int timeleft = lasts.get(ii) - hold;
				int dist = timeleft * hold;
				if (dist > records.get(ii)) {
					tmptot++;
				}
			}
			tot *= tmptot;
