...
after:
		...
		int max = 0;
		for (int yy3 = 0; yy3 < leny; yy3++) {
			for (int xx3 = 0; xx3 < lenx; xx3++) {
				if (grid[yy3][xx3] == '#' || grid[yy3][xx3] == ' ') {
					continue;
				}
				Arrays.stream(already).forEach(row -> Arrays.fill(row, 0));
				minPath = 9999;
				next(ex, ey, xx3, yy3, 0);
				if (minPath != 9999 && minPath > max) {max = minPath;}
			}
		}
