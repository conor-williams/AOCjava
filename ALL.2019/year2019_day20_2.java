...
class year2019_day20_2 {
	...
	public static void main(String [] args) {
		...
		already = new int[lAL+1][leny][lenx];
	...
	static void ne(int x, int y, int eX, int eY, int path, int ind) {
		if (x == eX && y == eY) {
			if (ind == lAL/2) {
		...
		if (iter == null) {} else {
			...
			if (ind == lAL/2 && iter.in == 2) {
				return;
			}
		
		}

		if (already[ind][y][x] == 0 || path < already[ind][y][x]) {
	...
	static void getsecond(char one1, char two1, int px1, int py1, int inside) {
		char fc2 = 0, sc2 = 0; int posX2 = 0, posY2 = 0; int in = 0;
after:
		for (int y = 0; y < leny-1; y++) {
			for (int x = 0; x < lenx-1; x++) {
				if ((Character.isUpperCase(grid[y][x])) && (Character.isUpperCase(grid[y+1][x]))) {
