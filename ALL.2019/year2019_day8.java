		...
		for (int layer = 0; layer < lMax; layer++) {
			...
			for (int yy = 0 ; yy < yMax; yy++) {
				for (int xx = 0; xx < xMax; xx++) {
					if (grid[layer][yy][xx] == 0) {
						count++;
					}
					//out.print(grid[layer][yy][xx]);
				}
				//out.println();
			}
			//out.println();
			if (count < countMin) {countMin = count; la = layer;}
		}
		int count2 = 0;
		int count1 = 0;
		for (int yy = 0 ; yy < yMax; yy++) {
